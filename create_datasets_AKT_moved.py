"""
Author: Antonia Jian
Date(Last modified): 29/10/2024
Description: 
This program is an extension of create_datasets_AKT_removal, with the following features:
    1. Move 50 non-SSD speakers from the train split to the test split, 
       aiming for an even age distribution.
    2. Keep all SSD speakers in the test split as in the original code.
...
def create_dataset_dict_AKT(data_dir, demographic_csv, annotation_file, output_dir):
    ...
    # Temporary lists for non-SSD speaker details
    non_ssd_speaker_datasets = []  # Stores datasets with non-SSD speaker information for age-based sampling
    non_ssd_speaker_details = []   # Stores IDs and ages for potential movement to test
    ...
    for _, row in handcorrected_df.iterrows():
        ...
        if str(speaker_id) in common_files:
            ...
            if ssd_status == 0:
            # Non-SSD group: only keep segments with error_count < 2 in train
                train_segments = [
                    segment for segment, error_count in zip(segments, error_counts)
                    if error_count < 2  # Only include low-error segments
                ]
                # We do not need test_segments in this case, as we're removing high-error samples
            else:
                # SSD group: all segments go to test
                test_segments = segments  # Include all segments regardless of error_count
    ...
    # Step to move 50 speakers from train to test
    # Organize non-SSD speakers by age and shuffle to ensure random selection within ages
    age_groups = collections.defaultdict(list)
    for speaker_id, age in non_ssd_speaker_details:
        age_groups[age].append((speaker_id, age)) 

     # Flatten age groups and shuffle for random selection
    evenly_distributed_speakers = [speaker for age_group in age_groups.values() for speaker in age_group]
    random.shuffle(evenly_distributed_speakers)

    # Select the first 50 unique speakers for moving to test
    speakers_to_move = evenly_distributed_speakers[:50]

    # Remove selected speakers from train_datasets and add to test_datasets
    moved_speaker_ids = []
    age_count = collections.Counter()
    for speaker_id, age in speakers_to_move:
        # Identify the dataset to move by speaker_id
        for dataset in train_datasets:
            if dataset["speaker_id"][0] == speaker_id:
                test_datasets.append(dataset)
                train_datasets.remove(dataset)
                moved_speaker_ids.append((speaker_id, age))
                age_count[age] += 1
                break
    ...
    print(f"Unique error counts: {sorted(all_error_counts)}")   # Print unique error counts
    ...
    print(f"Total IDs in train split after movement: {len(train_datasets)}")
    print(f"Total IDs in test split after movement: {len(test_datasets)}")
    print("Moved non-SSD IDs from train to test with ages:")
    for speaker_id, age in moved_speaker_ids:
        print(f" - Speaker ID: {speaker_id}, Age: {age}")
    print("Count of moved IDs by age:")
    for age, count in age_count.items():
        print(f" - Age {age}: {count} IDs")
...
output_directory = "/srv/scratch/z5369417/created_dataset_3009/AKT_move"
